	...
	public void randomtest() throws Throwable {
		...
		for (int iteration = 0; elapsed < TestTimeout; iteration++) {
			...
			try {
				...
				int numberSavedApp = ValuesGenerator.getRandomIntBetween(random, 1, 100);
				...
				for (int k = 0; k < numberSavedApp; k++) {
					...
					// Delete the appointment's information from the XML data tree.
					if(hit == 5) {
				...
				for (int i = 0; i < NUM_TESTS; i++) {
					try {
						...
						lastDay.add(firstDay.DAY_OF_MONTH, ValuesGenerator.getRandomIntBetween(random, -10, 10));
